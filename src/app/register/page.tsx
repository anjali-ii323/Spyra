...
import { Eye, EyeOff, Lock, Mail, User, Cpu } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    // If active session exists, redirect
    ...
    <div className="relative min-h-screen flex items-center justify-center p-6">
      ...
      <Card glow className="w-full max-w-[420px] bg-zinc-950/70 border-violet-500/20 shadow-[0_0_40px_rgba(139,92,246,0.04)]">
        <CardHeader className="text-center">
          ...
          <CardTitle>CREATE THREAT CONSOLE ACCOUNT</CardTitle>
          <CardDescription>
            Gain clearance to upload APKs and inspect malicious payloads.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="flex flex-col gap-5 font-mono text-xs">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-slate-400 uppercase">Analyst Full Name</label>
            ...
            <Button type="submit" variant="primary" fullWidth disabled={loading} className="mt-2">
              {loading ? (
                ...
                <>
                  PROVISION CLEARANCE <Cpu className="w-4 h-4" />
                </>
              )}
            </Button>

            <div className="text-center text-[10px] text-slate-500 mt-2 font-sans">
              Already have console access?{' '}
